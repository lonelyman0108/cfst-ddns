...
> v2 is a complete rewrite. Upgrading from v1 (the Bash scripts)? See the [migration guide](docs/MIGRATION.md) (Chinese).

## Features

- **Web UI**: dashboard, task management, live speed-test logs, run history and latency/speed trend charts; dark mode and mobile friendly
...
- **Smart updates**: skips unchanged IPs, keeps the existing record when a test finds nothing, can write the top N IPs for load balancing, and supports ISP lines, TTL and the Cloudflare proxy toggle
- **cfst management**: view and switch cfst versions, configure a GitHub mirror and edit the IP range files from the UI
- **Operations**: webhook triggers, config backup and restore, automatic history cleanup, encrypted credentials and a health check
...
Open `http://<host-ip>:8080` and set things up in this order:

1. Create the admin account
2. Add your provider credentials under **DNS Accounts** and test the connection
3. Optional: add a channel under **Notifications** and send a test message
4. Create a task under **Tasks**, pick the target records, then click **Run now** to watch the live log

> The web UI is currently in Chinese.
...
cfst is downloaded automatically on first start. If GitHub is unreachable from your network, set a GitHub mirror in **Settings**, then install cfst from the **cfst** page.
