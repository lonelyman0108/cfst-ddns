...
> v2 是完全重写的版本。从 v1（Bash 脚本）升级请阅读 [迁移指南](docs/MIGRATION.md)，也可以在首次初始化时直接导入旧的 `config.sh`。

## 功能

- **Web 界面**：仪表盘、任务管理、实时测速日志、执行历史、延迟/速度趋势图，支持暗色模式与移动端，服务商与渠道均配有品牌图标
- **四种界面语言**：English、简体中文、繁體中文、日本語，按浏览器语言自动选择，也可在顶栏切换
- **上手引导**：首次引导向导从安装 cfst 一路带到第一次试运行；仪表盘提供入门清单，另有可搜索的帮助页
...
- **智能更新**：IP 未变时跳过；测速无结果时保留原记录；可写入前 N 个 IP 做负载均衡；支持线路、TTL，以及 Cloudflare 代理开关；试运行只测速，不写 DNS、不发通知
- **cfst 管理**：自动下载或上传压缩包/二进制导入，自动识别已有 cfst，切换版本，GitHub 镜像测速，编辑 IP 段文件
- **运维**：Webhook 外部触发、配置备份与恢复、v1 配置导入、历史自动清理、凭据加密存储、健康检查
...
启动后打开 `http://<主机IP>:8080`，创建管理员账号（也可以在这里从备份恢复，或导入 v1 配置）。随后引导向导会带你完成：

1. 确认 cfst 已安装
2. 添加 DNS 账号并测试连接
3. 可选：添加通知渠道
4. 用模板创建第一个任务
5. 试运行一次，查看实时日志确认一切正常
...
首次启动时会自动下载 cfst。国内网络如果下载失败，先在「系统设置」中选择 GitHub 镜像（可一键测速），再到「cfst 管理」页安装；也可以在该页直接上传 cfst 压缩包。
