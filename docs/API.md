...
- 失败：4xx/5xx，响应体 `{"error": "错误信息"}`。401 表示未登录或令牌失效。
- 语言：请求头 `Accept-Language` 决定返回文案的语言，支持 `zh-CN`（默认）、`zh-TW`、`en`、`ja`；也可用查询参数 `?lang=`（优先于请求头，供 SSE 使用）。`zh-TW`/`zh-HK`/`zh-MO`/`zh-Hant` 归为繁体中文，其他 `zh*` 归为简体中文，无法识别时为简体中文。按语言翻译的内容：元数据接口的名称、说明、字段名、帮助、占位符与选项名，`error`、测试接口的 `message`、v1 导入的 `warnings` 与任务摘要、cfst 识别结果的 `message`、镜像的 `label`/`error`，以及按请求语言生成的默认名称（新建账号/渠道未填名称、克隆任务的「(副本)」后缀、v1 导入的账号/渠道/任务名称）。不翻译：执行日志、执行记录的 `message` 与变更说明、通知正文、服务商/上游返回的原始错误文本。
